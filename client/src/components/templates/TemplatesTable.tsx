        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {filteredTemplates.map((template) => {
            ...
            return (
              <div
                key={template.id}
                className="border rounded-lg p-4 hover:shadow-md transition-all cursor-pointer bg-[#0a0a0a] group"
                onClick={() => onViewTemplate(template)}
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-sm truncate text-[#e0e0e0]">
                      {template.name}
                    </h3>
                ...
                <div className="space-y-1.5">
                  ...
                  {template.footer && (
                    <p className="text-xs text-[#555] italic">{template.footer}</p>
                  )}
                </div>
                    ...
                    {buttons.map((button, idx) => {
                      ...
                      return (
                        <Badge
                          key={idx}
                          variant="outline"
                          className="text-[11px] px-1.5 py-0.5 bg-[#050505] text-[#999] border-[#1a1a1a]"
                        >
