...
const Header = () => {
  ...
  const MegaMenu = ({
    ...
    <div
      className={`fixed left-0 right-0 w-screen bg-[#0a0a0a]/80 dark:bg-gray-900/80 backdrop-blur-xl border-t border-emerald-100/50 dark:border-emerald-800/30 z-50 transition-all duration-400 ease-out max-h-[80vh] overflow-y-auto ${
        isVisible
    ...
    >
      <div className="max-w-7xl mx-auto p-5 sm:p-6 md:p-8">
          ...
          {items.map((item, index) => (
            ...
            >
              <div className="relative overflow-hidden rounded-xl mb-4">
                ...
                <div className="absolute bottom-2.5 right-2.5 bg-[#0a0a0a]/95 dark:bg-gray-800/95 backdrop-blur-md p-2 rounded-xl shadow-sm">
                  <item.icon className="w-4 h-4 text-emerald-600" />
                </div>
              </div>

              <h3 className="font-semibold text-[#e0e0e0] dark:text-white mb-1.5 group-hover:text-emerald-600 transition-colors text-[15px]">
                {item.title}
              </h3>
              <p className="text-[#555] dark:text-[#555] text-sm leading-relaxed line-clamp-2">
                {item.description}
              </p>
        ...
        <div className="mt-8 pt-6 border-t border-[#1a1a1a]/80 dark:border-gray-700/50 hidden md:block">
          <div className="bg-gradient-to-r from-emerald-50/80 to-teal-50/80 dark:from-emerald-900/15 dark:to-teal-900/15 rounded-2xl p-6 border border-emerald-100/50 dark:border-emerald-800/30">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                <h3 className="text-lg font-bold text-[#e0e0e0] dark:text-white mb-1">
                  {t("Landing.header.redystart")}
                </h3>
                <p className="text-[#999] dark:text-[#555] text-sm">
    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-40 transition-all duration-500 ease-out ${
          isScrolled
            ? "bg-[#0a0a0a]/70 dark:bg-gray-900/70 backdrop-blur-xl shadow-lg shadow-black/[0.03] border-b border-[#1a1a1a]/50 dark:border-gray-700/50"
            : "bg-[#0a0a0a]/90 dark:bg-gray-900/90 backdrop-blur-xl border-b border-transparent"
        }`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <nav className="hidden lg:flex items-center gap-1">
              <Link
                ...
                className={`relative px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 ${
                  location === "/"
                    ...
                    : "text-[#999] dark:text-gray-300 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-[#050505] dark:hover:bg-gray-800/50"
                }`}
              >
              ...
              {!isAuthenticated && (
                <div className="flex items-center gap-2 ml-2">
                  <Link
                    href="/login"
                    className="px-4 py-2 rounded-lg text-sm font-medium text-[#999] dark:text-gray-300 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-[#050505] dark:hover:bg-gray-800/50 transition-all duration-300"
                  >
              ...
              {isAuthenticated && (
                <div className="flex items-center gap-3 ml-2">
                  ...
                  <div className="relative" ref={dropdownRef}>
                    <button
                      className="w-10 h-10 rounded-xl overflow-hidden border-2 border-[#1a1a1a]/80 dark:border-gray-700 hover:border-emerald-300 dark:hover:border-emerald-600 transition-all duration-300 hover:shadow-md hover:shadow-emerald-500/10 ring-0 hover:ring-2 hover:ring-emerald-500/20"
                    ...
                    <div
                      className={`absolute right-0 mt-2 w-56 bg-[#0a0a0a]/90 dark:bg-gray-800/90 backdrop-blur-xl border border-[#1a1a1a]/80 dark:border-gray-700/80 rounded-2xl shadow-xl shadow-black/[0.08] z-50 overflow-hidden transition-all duration-300 origin-top-right ${
                        dropdownOpen
                          ? "opacity-100 scale-100 visible"
                          : "opacity-0 scale-95 invisible pointer-events-none"
                      }`}
                    >
                      <div className="px-4 py-3 border-b border-[#1a1a1a] dark:border-gray-700/80 bg-gradient-to-r from-emerald-50/50 to-transparent dark:from-emerald-900/10">
                        <p className="text-sm font-semibold text-[#e0e0e0] dark:text-white truncate">
                          {username}
                        </p>
                      </div>
                      ...
                      <div className="p-1.5">
                        <button
                          className="flex items-center w-full px-3 py-2.5 text-sm text-[#999] dark:text-gray-300 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 hover:text-emerald-600 dark:hover:text-emerald-400 rounded-xl transition-all duration-200"
                          onClick={() => {
                        ...
                        <div className="my-1 border-t border-[#1a1a1a] dark:border-gray-700/50"></div>
                        <button
            ...
            <button
              className="lg:hidden p-2.5 rounded-xl hover:bg-[#0a0a0a]/80 dark:hover:bg-gray-800/80 transition-all duration-300"
              onClick={handleMenuToggle}
              aria-label="Toggle menu"
              aria-expanded={isMenuOpen}
            >
              {isMenuOpen ? (
                <X className="w-5 h-5 text-[#999] dark:text-gray-300" />
              ) : (
                <Menu className="w-5 h-5 text-[#999] dark:text-gray-300" />
              )}
            </button>
          </div>
        </div>
      ...
      >
        ...
        <div
          className={`absolute top-0 right-0 w-full max-w-sm h-full bg-[#0a0a0a]/95 dark:bg-gray-900/95 backdrop-blur-xl border-l border-[#1a1a1a]/50 dark:border-gray-700/50 shadow-2xl transition-transform duration-500 ease-out overflow-y-auto ${
        ...
        >
          <div className="px-5 py-6 space-y-1">
            ...
            <div>
              <button
                className="flex items-center justify-between w-full px-4 py-3 rounded-xl text-[#e0e0e0] dark:text-white font-medium text-sm hover:bg-[#050505] dark:hover:bg-gray-800/50 transition-all duration-300"
                  ...
                  <div className="space-y-0.5 pl-4 pr-2 pb-2">
                    {staticData.header.aboutMenuItems.map((item, index) => (
                      <Link
                        ...
                        href={item.path}
                        className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-[#999] dark:text-[#555] hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-emerald-50/60 dark:hover:bg-emerald-900/15 font-medium text-sm transition-all duration-200"
                        onClick={() => setIsMenuOpen(false)}
                      >
            ...
            <div className="pt-4 mt-4 border-t border-[#1a1a1a] dark:border-gray-800 space-y-2">
              {!isAuthenticated ? (
                <>
                  <Link
                    href="/login"
                    ...
                    className="block w-full text-center px-4 py-3 rounded-xl text-[#999] dark:text-gray-300 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-[#050505] dark:hover:bg-gray-800/50 font-medium text-sm transition-all duration-300"
                  >
                    {loginLoading ? (
                ...
                <>
                  <Link
                    href="/dashboard"
                    className="block px-4 py-3 rounded-xl text-[#999] dark:text-gray-300 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-[#050505] dark:hover:bg-gray-800/50 font-medium text-sm transition-all duration-300"
                    onClick={() => setIsMenuOpen(false)}
                  >
                    {t("Landing.header.dash")}
                  </Link>

                  <div className="border-t border-[#1a1a1a] dark:border-gray-800 pt-4 mt-2">
                    <div className="flex items-center gap-3 px-4 mb-3">
                      ...
                      <span className="text-sm text-[#e0e0e0] dark:text-white font-semibold">
                        {username}
                      </span>
                    </div>
                    ...
                    <div className="space-y-0.5">
                      <button
                        className="flex items-center w-full px-4 py-2.5 text-sm text-[#999] dark:text-gray-300 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 hover:text-emerald-600 dark:hover:text-emerald-400 rounded-xl transition-all duration-200"
                        onClick={() => {
                          setLocation("/settings");
                          setIsMenuOpen(false);
                        }}
