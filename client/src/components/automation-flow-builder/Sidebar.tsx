...
export function Sidebar({ onAddNode }: SidebarProps) {
  return (
    <TooltipProvider delayDuration={200}>
      <div className="col-span-2 bg-[#0a0a0a] flex flex-col border-r border-[#1a1a1a] overflow-hidden">
        <div className="px-3 py-2.5 border-b border-[#1a1a1a]">
          <div className="font-semibold text-xs text-[#e0e0e0]">Flow Nodes</div>
          <div className="text-[10px] text-[#555] mt-0.5">Click to add · hover <Info className="w-2.5 h-2.5 inline" /> for help</div>
        </div>

        <div className="flex-1 overflow-y-auto px-2 py-2 space-y-2.5">
          {nodeCategories.map((category) => (
            <div key={category.label}>
              <div className="text-[9px] font-bold uppercase tracking-wider text-[#555] mb-1.5 px-0.5">
                {category.label}
              </div>
                  ...
                  return (
                    <div key={item.kind} className="relative">
                      ...
                      >
                        ...
                        <span className="text-[10px] font-medium text-[#999] leading-tight text-center w-full">
                          {item.name}
                        </span>
                      </button>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <button
                            className="absolute top-1 right-1 p-0.5 rounded-full hover:bg-[#0a0a0a]/80 transition-colors"
                            onClick={(e) => e.stopPropagation()}
                          >
                            <Info className="w-3 h-3 text-[#555] hover:text-[#999]" />
                          </button>
                        </TooltipTrigger>
                        <TooltipContent side="right" className="max-w-[220px] text-xs leading-relaxed p-3">
                          <p className="font-semibold mb-1">{item.name}</p>
                          <p className="text-[#555]">{item.tip}</p>
                        </TooltipContent>
                      </Tooltip>
                    </div>
                  );
