...
function NodeShell({
  ...
  return (
    <div
      className={`rounded-xl bg-[#0a0a0a] shadow-md min-w-[240px] max-w-[280px] overflow-hidden transition-all duration-200 border ${
        selected
          ? "border-blue-400 ring-2 ring-blue-100"
          : "border-[#1a1a1a] hover:shadow-lg"
      }`}
    >
      <div className={`flex items-center gap-2.5 px-3.5 py-2.5 ${bgColor} border-b ${borderColor}`}>
        <div className={`w-7 h-7 rounded-lg bg-[#0a0a0a]/80 flex items-center justify-center ${color} shrink-0`}>
          {icon}
        </div>
        <span className={`font-semibold text-xs ${color}`}>{title}</span>
      </div>
      {children && (
        <div className="px-3.5 py-2.5 text-xs text-[#999] space-y-2">
          {children}
        </div>
      )}
    </div>
...
export function StartNode() {
  return (
    <div className="relative flex flex-col items-center">
      ...
      <div className="mt-1.5 px-2.5 py-0.5 bg-[#0a0a0a] rounded-full shadow-sm border border-[#1a1a1a]">
    ...
    <div className="relative">
      ...
      >
        {data.conditionType === "keyword" && data.keywords && data.keywords.length > 0 ? (
          ...
          <div className="text-[#555] italic text-[11px]">No conditions set</div>
        )}
        {data.matchType && (
...
export function CustomReplyNode({ data }: { data: BuilderNodeData }) {
  return (
    <div className="relative">
      ...
      >
        {data.message ? (
          <p className="line-clamp-2 text-[11px] text-[#999] bg-[#050505] rounded-lg p-2 border border-[#1a1a1a]">
            {data.message.length > 80 ? `${data.message.slice(0, 80)}...` : data.message}
          </p>
        ) : (
          <div className="text-[#555] italic text-[11px]">No message set</div>
        )}

        <div className="flex flex-wrap gap-1">
          {data.imagePreview && (
            <span className="inline-flex items-center gap-0.5 bg-[#0a0a0a] text-[#555] text-[10px] px-1.5 py-0.5 rounded font-medium">
        ...
        {data.buttons && data.buttons.length > 0 && (
          <div className="flex flex-wrap gap-1 pt-1.5 border-t border-[#1a1a1a]">
    ...
    <div className="relative">
      ...
      >
        {data.question ? (
          ...
          <div className="text-[#555] italic text-[11px]">No question set</div>
        )}
        {data.saveAs && (
...
export function TimeGapNode({ data }: { data: BuilderNodeData }) {
  ...
  return (
    <div className="relative">
      ...
      >
        <div className="flex items-center gap-2 bg-[#050505] rounded-lg p-2 border border-[#1a1a1a]">
          <span className="text-lg font-bold text-slate-700">{display}</span>
          <span className="text-[10px] text-[#555] font-medium uppercase">pause</span>
        </div>
      </NodeShell>
      ...
      >
        {data.templateId ? (
          <div className="flex items-center gap-2 bg-[#050505] rounded-lg p-2 border border-[#1a1a1a]">
            ...
            <span className="text-[11px] text-[#999] font-medium">Template selected</span>
          </div>
        ) : (
          <div className="text-[#555] italic text-[11px]">No template selected</div>
        )}
      </NodeShell>
      ...
      >
        {data.assigneeId ? (
          <div className="flex items-center gap-2 bg-[#050505] rounded-lg p-2 border border-[#1a1a1a]">
            ...
            <span className="text-[11px] text-[#999] font-medium">Agent assigned</span>
          </div>
        ) : (
          <div className="text-[#555] italic text-[11px]">No agent selected</div>
        )}
      </NodeShell>
      ...
      >
        {data.webhookUrl ? (
          <div className="space-y-1.5">
            ...
            <div className="text-[11px] text-[#555] truncate bg-[#050505] rounded px-2 py-1 font-mono border border-[#1a1a1a]">{data.webhookUrl}</div>
          </div>
        ) : (
          <div className="text-[#555] italic text-[11px]">No webhook configured</div>
        )}
      </NodeShell>
      ...
      >
        {data.groupName ? (
          <div className="flex items-center gap-2 bg-[#050505] rounded-lg p-2 border border-[#1a1a1a]">
            ...
            <span className="text-[11px] text-[#999] font-medium truncate">{data.groupName}</span>
          </div>
        ) : (
          <div className="text-[#555] italic text-[11px]">No group selected</div>
        )}
      </NodeShell>
        ...
        {data.contactField ? (
          <div className="space-y-1">
            ...
            {data.contactFieldValue && (
              <div className="text-[11px] text-[#555] truncate bg-[#050505] rounded px-2 py-1 border border-[#1a1a1a]">
                {data.contactFieldValue}
              </div>
            )}
          </div>
        ) : (
          <div className="text-[#555] italic text-[11px]">No field configured</div>
        )}
      </NodeShell>
    ...
    <div className="relative">
      ...
      >
        {data.variableName ? (
          ...
          <div className="text-[#555] italic text-[11px]">No variable set</div>
        )}
      </NodeShell>
    ...
    <div className="relative">
      ...
      >
        {data.locationName || (data.latitude && data.longitude) ? (
          <div className="space-y-1">
            {data.locationName && (
              <div className="text-[11px] text-[#999] font-medium">{data.locationName}</div>
            )}
            {data.latitude && data.longitude && (
              <div className="text-[10px] text-[#555] font-mono bg-[#050505] rounded px-2 py-1 border border-[#1a1a1a]">
                {data.latitude}, {data.longitude}
              </div>
            )}
          </div>
        ) : (
          <div className="text-[#555] italic text-[11px]">No location set</div>
        )}
      </NodeShell>
    ...
    <div className="relative">
      ...
      >
        {data.message ? (
          ...
          <div className="text-[#555] italic text-[11px]">No body text</div>
        )}
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-[#555]">{totalRows} items</span>
        </div>
      </NodeShell>
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {data.mediaUrl ? (
          <div className="text-[11px] text-[#555] truncate bg-[#050505] rounded px-2 py-1 font-mono border border-[#1a1a1a]">
            {data.mediaUrl}
          </div>
        ) : (
          <div className="text-[#555] italic text-[11px]">No media URL set</div>
        )}
        {data.mediaCaption && (
          <p className="text-[10px] text-[#555] truncate">{data.mediaCaption}</p>
        )}
      </NodeShell>
    ...
    <div className="relative">
      ...
      >
        <div className="flex items-center gap-2 bg-[#050505] rounded-lg p-2 border border-[#1a1a1a]">
          ...
          <span className="text-[11px] text-[#999] font-medium">Send read receipts</span>
        </div>
      </NodeShell>
