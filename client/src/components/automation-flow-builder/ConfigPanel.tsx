...
function SectionHeader({ children }: { children: React.ReactNode }) {
  return (
    <div className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#555] mt-1">{children}</div>
  );
}
...
export function ConfigPanel({
  ...
  if (!selected || selected.data.kind === "start") {
    return (
      <div className="h-full flex flex-col items-center justify-center px-6 text-center">
        <div className="w-12 h-12 rounded-lg bg-[#0a0a0a] flex items-center justify-center mb-3">
          <Settings2 className="w-5 h-5 text-[#555]" />
        </div>
        <div className="font-medium text-sm text-[#999]">Node Properties</div>
        <div className="text-xs text-[#555] mt-1">Click any node on the canvas to edit its settings</div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0a0a]">
      <div className={`px-4 py-3 ${meta.bgTint} border-b border-[#1a1a1a]`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className={`w-8 h-8 rounded-lg ${meta.bgTint} flex items-center justify-center border border-[#1a1a1a]`}>
              <Icon className={`w-4 h-4 ${meta.color}`} />
            </div>
            <div>
              <div className="font-semibold text-sm text-[#e0e0e0]">{meta.label}</div>
              <div className="text-[10px] text-[#555] font-mono truncate max-w-[120px]">{selected.id}</div>
            </div>
          </div>
          {d.kind !== "start" && (
            <Button size="sm" variant="ghost" onClick={onDelete} className="h-7 w-7 p-0 text-[#555] hover:text-red-500 hover:bg-red-50 rounded-lg">
            ...
            <>
              ...
              <div className="space-y-3 bg-purple-50/50 rounded-xl p-4 border border-purple-100">
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">Condition Type</Label>
                  <Select value={d.conditionType || "keyword"} onValueChange={(v) => onChange({ conditionType: v as any })}>
                    <SelectTrigger className="h-9 text-sm bg-[#0a0a0a]"><SelectValue /></SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">Match Type</Label>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label className="text-xs font-semibold text-[#999]">Keywords</Label>
                ...
                {(!d.keywords || d.keywords.length === 0) && (
                  <div className="text-xs text-[#555] italic py-3 text-center bg-[#050505] rounded-lg">No keywords added yet</div>
                )}
              </div>
            </>
          )}
          ...
          {d.kind === "custom_reply" && (
            <>
              ...
              <div className="space-y-3 bg-blue-50/50 rounded-xl p-4 border border-blue-100">
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">Message Text</Label>
                  <Textarea rows={4} value={d.message || ""} onChange={(e) => onChange({ message: e.target.value })} placeholder="Type your message..." className="text-sm resize-none rounded-lg bg-[#0a0a0a]" />
                </div>
              </div>
              ...
              {d.imagePreview && (
                <div className="relative rounded-xl overflow-hidden border border-[#1a1a1a] shadow-sm">
              ...
              {d.documentPreview && (
                <div className="flex items-center gap-2 bg-[#050505] border border-[#1a1a1a] rounded-xl p-3">
                  <FileIcon className="w-4 h-4 text-[#555]" />
                  <span className="text-xs flex-1 font-medium text-[#999]">Document attached</span>
          ...
          {d.kind === "user_reply" && (
            <>
              ...
              <div className="space-y-3 bg-amber-50/50 rounded-xl p-4 border border-amber-100">
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">Question Text</Label>
                  <Textarea rows={3} value={d.question || ""} onChange={(e) => onChange({ question: e.target.value })} placeholder="Enter question to ask..." className="text-sm resize-none rounded-lg bg-[#0a0a0a]" />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">Save As Variable</Label>
                  <Input value={d.saveAs || ""} onChange={(e) => onChange({ saveAs: e.target.value })} placeholder="e.g., transfer_consent" className="h-8 text-sm font-mono rounded-lg bg-[#0a0a0a]" />
                  <div className="text-[10px] text-[#555]">Use lowercase with underscores</div>
                </div>
              </div>
          ...
          {d.kind === "time_gap" && (
            <>
              ...
              <div className="space-y-3 bg-slate-50/80 rounded-xl p-4 border border-slate-200">
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">Delay Duration (seconds)</Label>
                  <Input type="number" min={10} value={d.delay ?? 60} onChange={(e) => onChange({ delay: parseInt(e.target.value, 10) })} className="h-9 text-sm rounded-lg bg-[#0a0a0a]" />
                  <div className="text-[10px] text-[#555]">Min 10 seconds. Flow pauses before the next step.</div>
                </div>
              </div>
            </>
          )}
          ...
          {d.kind === "send_template" && (
            <>
              ...
              <div className="space-y-3 bg-teal-50/50 rounded-xl p-4 border border-teal-100">
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">Choose Template</Label>
                  <Select
                    value={d.templateId || ""}
                  ...
                  >
                    <SelectTrigger className="h-9 text-sm bg-[#0a0a0a] rounded-lg"><SelectValue placeholder="Select template" /></SelectTrigger>
                    <SelectContent>
                      {templates.map((t) => (
              ...
              {bodyVarsArray.length > 0 && (
                <div className="space-y-3">
                  ...
                  {bodyVarsArray.map((varText: string) => {
                    ...
                    return (
                      <div key={index} className="space-y-1.5 bg-[#050505] rounded-xl p-3 border border-[#1a1a1a]">
                        <Label className="text-[11px] text-[#555] font-semibold">{varText}</Label>
                        <Select
                        ...
                        >
                          <SelectTrigger className="h-8 text-xs bg-[#0a0a0a] rounded-lg"><SelectValue placeholder="Select source" /></SelectTrigger>
                          <SelectContent>
                        ...
                        {sampleValue && <p className="text-[10px] text-[#555]">Sample: <span className="font-semibold">{sampleValue}</span></p>}
                      </div>
                    );
                  })}
                </div>
          ...
          {d.kind === "assign_user" && (
            <>
              ...
              <div className="space-y-3 bg-indigo-50/50 rounded-xl p-4 border border-indigo-100">
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999] flex items-center gap-1.5">
                    <Users className="w-3.5 h-3.5 text-indigo-500" /> Select Agent
                  </Label>
                  <Select value={d.assigneeId || ""} onValueChange={(v) => onChange({ assigneeId: v })}>
                    <SelectTrigger className="h-9 text-sm bg-[#0a0a0a] rounded-lg"><SelectValue placeholder="Select agent" /></SelectTrigger>
                    <SelectContent>
                      {members.map((m) => (
          ...
          {d.kind === "webhook" && (
            <>
              ...
              <div className="space-y-3 bg-orange-50/50 rounded-xl p-4 border border-orange-100">
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">HTTP Method</Label>
                  <Select value={d.webhookMethod || "POST"} onValueChange={(v) => onChange({ webhookMethod: v as any })}>
                    <SelectTrigger className="h-9 text-sm bg-[#0a0a0a] rounded-lg"><SelectValue /></SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">Webhook URL</Label>
                  <Input value={d.webhookUrl || ""} onChange={(e) => onChange({ webhookUrl: e.target.value })} placeholder="https://api.example.com/webhook" className="h-9 text-sm rounded-lg bg-[#0a0a0a]" />
                </div>
              </div>
              ...
              {(d.webhookMethod === "POST" || d.webhookMethod === "PUT" || !d.webhookMethod) && (
                <>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="text-[10px] text-[#555]">Leave empty to send full contact & conversation data automatically. Or use variables below to build a custom body.</div>
                  </div>
                </>
              )}

              {d.webhookMethod === "GET" && (
                <div className="text-[10px] text-[#555] bg-orange-50/50 rounded-lg p-3 border border-orange-100">
          ...
          {d.kind === "end" && (
            <>
              ...
              <div className="space-y-3 bg-red-50/50 rounded-xl p-4 border border-red-100">
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">End Label (Optional)</Label>
                  <Input value={d.endMessage || ""} onChange={(e) => onChange({ endMessage: e.target.value })} placeholder="e.g., Conversation ended" className="h-9 text-sm rounded-lg bg-[#0a0a0a]" />
                  <div className="text-[10px] text-[#555]">Displayed on the End node in the canvas</div>
                </div>
              </div>
            </>
          )}
            ...
            <>
              ...
              <div className="space-y-3 bg-emerald-50/50 rounded-xl p-4 border border-emerald-100">
                <div className="space-y-1.5">
                  ...
                  >
                    <SelectTrigger className="h-9 text-sm bg-[#0a0a0a] rounded-lg"><SelectValue placeholder="Select a group" /></SelectTrigger>
                    <SelectContent>
                  ...
                  {(contactGroups as any[]).length === 0 && (
                    <div className="text-[10px] text-[#555] italic">No groups found. Create groups in Contact Management first.</div>
                  )}
                </div>
              </div>
            </>
          ...
          {d.kind === "update_contact" && (
            <>
              ...
              <div className="space-y-3 bg-cyan-50/50 rounded-xl p-4 border border-cyan-100">
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">Field to Update</Label>
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">New Value</Label>
                  <Input
                    value={d.contactFieldValue || ""}
                    onChange={(e) => onChange({ contactFieldValue: e.target.value })}
                    placeholder="Enter value or use {{variable}}"
                    className="h-9 text-sm rounded-lg bg-[#0a0a0a]"
                  />
                  <div className="text-[10px] text-[#555]">Use {"{{variable_name}}"} to insert flow variable values</div>
                </div>
              </div>
            </>
          )}
          ...
          {d.kind === "set_variable" && (
            <>
              ...
              <div className="space-y-3 bg-violet-50/50 rounded-xl p-4 border border-violet-100">
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">Variable Name</Label>
                  <Input
                    value={d.variableName || ""}
                    onChange={(e) => onChange({ variableName: e.target.value })}
                    placeholder="e.g., user_category"
                    className="h-9 text-sm font-mono rounded-lg bg-[#0a0a0a]"
                  />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">Value Source</Label>
                ...
                {d.variableSource === "static" && (
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold text-[#999]">Value</Label>
                    <Input
                ...
                {d.variableSource === "from_webhook" && (
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold text-[#999]">JSON Path</Label>
                    <Input
                      value={d.variableValue || ""}
                      onChange={(e) => onChange({ variableValue: e.target.value })}
                      ...
                      className="h-9 text-sm font-mono rounded-lg bg-[#0a0a0a]"
                    />
                    <div className="text-[10px] text-[#555]">Dot-notation path from webhook response JSON</div>
                  </div>
                )}
              </div>
            </>
          ...
          {d.kind === "send_location" && (
            <>
              ...
              <div className="space-y-3 bg-rose-50/50 rounded-xl p-4 border border-rose-100">
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">Location Name</Label>
                  <Input
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">Address</Label>
                  <Input
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold text-[#999]">Latitude</Label>
                    <Input
                      value={d.latitude || ""}
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold text-[#999]">Longitude</Label>
                    <Input
                      value={d.longitude || ""}
          ...
          {d.kind === "send_list_message" && (
            <>
              ...
              <div className="space-y-3 bg-sky-50/50 rounded-xl p-4 border border-sky-100">
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">Body Text</Label>
                  <Textarea
                    rows={3}
                    value={d.message || ""}
                    onChange={(e) => onChange({ message: e.target.value })}
                    placeholder="Message shown above the list button"
                    className="text-sm resize-none rounded-lg bg-[#0a0a0a]"
                  />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">Button Text</Label>
                  <Input
                    ...
                    maxLength={20}
                  />
                  <div className="text-[10px] text-[#555]">Max 20 characters</div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                {(d.listSections || []).map((section, sIdx) => (
                  <div key={sIdx} className="bg-[#050505] rounded-xl p-3 border border-[#1a1a1a] space-y-2">
          ...
          {d.kind === "send_media" && (
            <>
              ...
              <div className="space-y-3 bg-pink-50/50 rounded-xl p-4 border border-pink-100">
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">Media Type</Label>
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#999]">Source</Label>
                  <div className="flex gap-1 p-0.5 bg-[#0a0a0a] rounded-lg">
                    <button
                      type="button"
                      className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 px-2 text-xs font-medium rounded-md transition-all ${(d.mediaSourceType || "url") === "url" ? "bg-[#0a0a0a] shadow-sm text-[#e0e0e0]" : "text-[#555] hover:text-[#999]"}`}
                    ...
                    <button
                      type="button"
                      className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 px-2 text-xs font-medium rounded-md transition-all ${d.mediaSourceType === "upload" ? "bg-[#0a0a0a] shadow-sm text-[#e0e0e0]" : "text-[#555] hover:text-[#999]"}`}
                ...
                {(d.mediaSourceType || "url") === "url" ? (
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold text-[#999]">Media URL</Label>
                    <Input
                      value={d.mediaUrl || ""}
                      ...
                      className="h-9 text-sm rounded-lg bg-[#0a0a0a]"
                    />
                    <div className="text-[10px] text-[#555]">Direct link to the media file</div>
                  </div>
                ) : (
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold text-[#999]">Upload File</Label>
                    {d.mediaId ? (
                      <div className="flex items-center gap-2 p-2 bg-[#0a0a0a] rounded-lg border border-pink-200">
                        <Paperclip className="w-3.5 h-3.5 text-pink-500 shrink-0" />
                        <span className="text-xs text-[#999] truncate flex-1">{d.mediaFileName || "Uploaded file"}</span>
                        <Button
                          size="sm"
                          variant="ghost"
                          className="h-6 w-6 p-0 text-[#555] hover:text-red-500"
                        ...
                        >
                          {mediaUploading ? (
                            ...
                            <>
                              ...
                              <span className="text-xs text-[#555]">Click to upload {d.mediaType || "image"}</span>
                            </>
                          )}
                        </label>
                      </div>
                ...
                {(d.mediaType === "image" || d.mediaType === "video" || d.mediaType === "document") && (
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold text-[#999]">Caption (Optional)</Label>
                    <Textarea
                      rows={2}
            ...
            <>
              ...
              <div className="space-y-2 bg-lime-50/50 rounded-xl p-4 border border-lime-100">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-[#999]">Sends read receipts (blue ticks) for the last incoming message from the customer.</span>
                </div>
                <div className="text-[10px] text-[#555]">No configuration needed. This node automatically marks the conversation as read.</div>
              </div>
            </>
          )}
