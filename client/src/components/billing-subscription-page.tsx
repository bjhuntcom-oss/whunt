...
export default function BillingSubscriptionPage({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  if (isLoading) {
    return (
      <div className={embedded ? "flex items-center justify-center p-4" : "flex-1 min-h-screen flex items-center justify-center p-4 bg-[#0a0a0a] text-[#999]"}>
  ...
  return (
    <div className={embedded ? "" : "flex-1 bg-[#0a0a0a] text-[#e0e0e0] dots-bg"}>
      <div className="p-6 pb-0 bg-[#0a0a0a] border">
        ...
        <p className="text-[#555] text-sm ml-14 pb-2">
      ...
      <main className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 p-6">
        ...
        activeplandata.data.length === 0 ? (
          <div className="col-span-full flex flex-col items-center justify-center py-16 bg-[#0a0a0a] rounded-2xl border border-[#1a1a1a] shadow-sm">
            <div className="p-4 bg-[#0a0a0a] rounded-full">
              <svg
                className="w-10 h-10 text-[#555]"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
                viewBox="0 0 24 24"
          ...
          activeplandata.data.map((item) => {
            ...
            return (
              <div
                key={subscription.id}
                className="bg-[#0a0a0a] rounded-xl shadow-lg border border-[#1a1a1a] hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col h-full max-w-sm"
              >
                ...
                <div className="p-4 flex-grow flex flex-col space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    <div className="bg-blue-50 rounded-lg p-2 text-center">
                      ...
                      <p className="font-semibold text-[#e0e0e0] capitalize truncate">
                    ...
                    <div className="bg-green-50 rounded-lg p-2 text-center">
                      ...
                      <p className="font-semibold text-[#e0e0e0] text-[10px]">
                        {startDate}
                      </p>
                    </div>
                  ...
                  {/* Pricing */}
                  <div>
                    <h3 className="text-xs font-semibold text-[#999] mb-2">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {Object.entries(planData.permissions).map(
                          ([key, value]) => (
                            <div
                              key={key}
                              className="flex items-center gap-1 text-[10px] bg-[#0a0a0a] rounded-md px-2 py-1"
                            >
                              <Check className="w-3 h-3 text-green-600" />
                              <span className="text-[#999] capitalize">
                                {value} {key}
                              </span>
                            </div>
                          )
                      ...
                      {planData.features.map((feature, idx) => (
                        ...
                        >
                          {feature.included ? (
                            ...
                            <X className="w-3.5 h-3.5 text-[#555] flex-shrink-0 mt-0.5" />
                          )}
                ...
                {subscription.status === "active" && (
                  <div className="p-4 pt-2 space-y-2 border-t border-[#1a1a1a]">
                    <Button
                      variant="outline"
                      className="w-full"
                      onClick={() => setLocation("/plan-upgrade")}
