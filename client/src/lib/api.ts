...
// INC-01 FIX: Define proper types instead of using 'any'
...
getAllContacts: (
  ...
  createContact: (data: ContactData, channelId?: string) => apiRequest("POST", `/api/contacts${channelId ? `?channelId=${channelId}` : ""}`, data),
  updateContact: (id: string, data: Partial<ContactData>) => apiRequest("PUT", `/api/contacts/${id}`, data),
  ...
  createCampaign: (data: CampaignData, channelId?: string) => apiRequest("POST", `/api/campaigns${channelId ? `?channelId=${channelId}` : ""}`, data),
  updateCampaign: (id: string, data: Partial<CampaignData>) => apiRequest("PUT", `/api/campaigns/${id}`, data),
  ...
  createConversation: (data: ConversationData) => apiRequest("POST", "/api/conversations", data),
  updateConversation: (id: string, data: Partial<ConversationData>) => apiRequest("PUT", `/api/conversations/${id}`, data),

  // Messages
  getMessages: (conversationId: string) => apiRequest("GET", `/api/conversations/${conversationId}/messages`),
  createMessage: (conversationId: string, data: MessageData) => apiRequest("POST", `/api/conversations/${conversationId}/messages`, data),
  ...
  createAutomation: (data: AutomationData, channelId?: string) => apiRequest("POST", `/api/automations${channelId ? `?channelId=${channelId}` : ""}`, data),
  updateAutomation: (id: string, data: Partial<AutomationData>) => apiRequest("PUT", `/api/automations/${id}`, data),
  ...
  getChatbot: (id: string) => apiRequest("GET", `/api/chatbots/${id}`),
  updateChatbot: (id: string, data: unknown) => apiRequest("PUT", `/api/chatbots/${id}`, data),
  deleteChatbot: (id: string) => apiRequest("DELETE", `/api/chatbots/${id}`),

  // ====================== TRAINING DATA ======================
  addTrainingData: (data: unknown) => apiRequest("POST", "/api/training-data", data),
  getTrainingData: (chatbotId: string) => apiRequest("GET", `/api/training-data/${chatbotId}`),
