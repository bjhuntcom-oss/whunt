...
const getContentTypeIcon = (contentType: string) => {
  switch (contentType?.toLowerCase()) {
    ...
    default:
      return <Type className="w-4 h-4 text-[#555]" />;
  }
};
...
const getContentTypeBadgeColor = (contentType: string) => {
  switch (contentType?.toLowerCase()) {
    ...
    default: return "bg-[#050505] text-[#999] border-[#1a1a1a]";
  }
};
...
function MessageDetailDialog({ log, open, onClose, isDemo }: { log: MessageLog | null; open: boolean; onClose: () => void; isDemo: boolean }) {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "sent": return <Clock className="w-4 h-4 text-[#999]" />;
      case "pending": return <Clock className="w-4 h-4 text-yellow-600" />;
      default: return <MessageSquare className="w-4 h-4 text-[#999]" />;
    }
  };
  ...
  const renderMediaPreview = () => {
    ...
    if (ct === "image") {
      return (
        <div className="mt-3">
          <p className="text-xs font-medium text-[#555] mb-1">Media Preview</p>
          {url ? (
            <img src={url} alt="Message image" className="max-w-full max-h-64 rounded-lg border object-contain" />
          ) : (
            <div className="flex items-center gap-2 text-sm text-[#555] bg-[#050505] p-3 rounded-lg">
    ...
    if (ct === "document") {
      return (
        <div className="mt-3">
          <p className="text-xs font-medium text-[#555] mb-1">Document</p>
          <div className="flex items-center gap-2 text-sm bg-[#050505] p-3 rounded-lg border">
            <FileText className="w-5 h-5 text-yellow-600" />
            <div className="flex-1">
              <span className="text-[#999]">{log.content || "Document"}</span>
              {log.mediaMimeType && <span className="text-xs text-[#555] ml-2">({log.mediaMimeType})</span>}
            </div>
            {url && (
    ...
    if (ct === "sticker") {
      return (
        <div className="mt-3">
          <p className="text-xs font-medium text-[#555] mb-1">Sticker</p>
          {url ? (
      ...
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <p className="text-xs font-medium text-[#555]">Type</p>
            ...
            <div>
              <p className="text-xs font-medium text-[#555]">Status</p>
              <div className="flex items-center gap-1 text-sm">
          ...
          <div>
            <p className="text-xs font-medium text-[#555] mb-1">Content</p>
            <div className="bg-[#050505] p-3 rounded-lg text-sm whitespace-pre-wrap break-words border">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              <p className="text-xs font-medium text-[#555]">Created</p>
              <p>{format(new Date(log.createdAt), "MMM d, yyyy h:mm:ss a")}</p>
            </div>
            {log.deliveredAt && (
              <div>
                <p className="text-xs font-medium text-[#555]">Delivered</p>
                <p>{format(new Date(log.deliveredAt), "MMM d, yyyy h:mm:ss a")}</p>
              </div>
            )}
            {log.readAt && (
              <div>
                <p className="text-xs font-medium text-[#555]">Read</p>
          ...
          {log.whatsappMessageId && (
            <div>
              <p className="text-xs font-medium text-[#555]">WhatsApp Message ID</p>
              <p className="text-xs font-mono text-[#999] break-all bg-[#050505] p-2 rounded">{log.whatsappMessageId}</p>
            </div>
          )}

          {log.templateName && (
            <div>
              <p className="text-xs font-medium text-[#555]">Template</p>
          ...
          {log.metadata && Object.keys(log.metadata).length > 0 && (
            <div>
              <p className="text-xs font-medium text-[#555] mb-1">Metadata</p>
              <pre className="bg-[#050505] p-2 rounded text-xs overflow-x-auto border max-h-40">
...
export default function SuperadminMessageLogs() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      default:
        return <MessageSquare className="w-4 h-4 text-[#999]" />;
    }
  };
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "sent":
      case "pending":
        return "bg-[#0a0a0a] text-[#e0e0e0] border-[#252525]";
      default:
        return "bg-[#0a0a0a] text-[#e0e0e0] border-[#252525]";
    }
  };
          ...
          <CardContent>
            ...
            {isLoading ? (
              ...
              <div className="text-center py-12">
                <MessageSquare className="w-12 h-12 mx-auto text-[#555] mb-4" />
                <p className="text-[#555] mb-2">{t("messageLogs.noLogs") !== "messageLogs.noLogs" ? t("messageLogs.noLogs") : "No message logs found"}</p>
                    ...
                    <TableBody>
                      {logs.map((log) => (
                        ...
                        >
                          ...
                          <TableCell className="text-sm text-[#999] whitespace-nowrap">
                ...
                <div className="mt-4 flex flex-col sm:flex-row items-center justify-between gap-3 text-sm">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {getPageNumbers().map((p, i) =>
                      p === "..." ? (
                        <span key={`ellipsis-${i}`} className="px-2 text-[#555]">...</span>
                      ) : (
                        <Button
                          key={p}
