      ...
      <div className="p-4 md:p-6">
          ...
          <>
            ...
            >
              <div className="relative flex-1">
                ...
                />
                <FaSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-[#555]" />
              </div>
              <select
                ...
                className="border border-[#252525] rounded-md px-3 py-2 text-sm bg-[#0a0a0a] w-full sm:w-auto"
              >
            ...
            <div className="mb-4 text-sm text-[#999]">
              Showing {channels.length} of {channelPagination.total} channels
            </div>

            <div className="hidden md:block bg-[#0a0a0a] border rounded-lg shadow-sm overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-100">
                  {channelsLoading ? (
                    <tr>
                      <td colSpan={8} className="py-10 text-center text-[#555]">
                        Loading channels...
                      </td>
                    </tr>
                  ) : channels.length === 0 ? (
                    ...
                    channels.map((ch) => (
                      <tr key={ch.id} className="hover:bg-[#050505]">
                        ...
                        <td className="px-4 py-3">
                          <div>
                            {ch.createdBy ? (
                              ...
                              <span className="text-sm text-[#555]">Unknown</span>
                            )}
                            {ch.ownerEmail && (
                              <p className="text-xs text-[#555]">{_isDemo ? maskEmail(ch.ownerEmail) : ch.ownerEmail}</p>
                            )}
                          </div>
                        </td>
                        <td className="px-4 py-3">
            ...
            <div className="md:hidden space-y-4">
              {channelsLoading ? (
                <p className="text-center text-[#555] py-10">Loading channels...</p>
              ) : channels.length === 0 ? (
                <p className="text-center text-[#555] py-10">No channels found</p>
              ) : (
                channels.map((ch) => (
                  <div
                    key={ch.id}
                    className="bg-[#0a0a0a] shadow-sm rounded-lg p-4 border space-y-3"
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-semibold text-[#e0e0e0]">{_isDemo ? maskName(ch.name || "Unnamed") : (ch.name || "Unnamed")}</p>
                        <p className="text-xs text-[#555]">{_isDemo ? maskPhone(ch.phoneNumber) : (ch.phoneNumber || "N/A")}</p>
                      </div>
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-semibold ${
                          ch.isActive
                    ...
                    <div className="text-sm text-[#999] space-y-1">
                      <p>
                        ...
                        {ch.ownerEmail && (
                          <span className="text-xs text-[#555] ml-1">({_isDemo ? maskEmail(ch.ownerEmail) : ch.ownerEmail})</span>
                        )}
                      </p>
                      <p>
        ...
        {activeTab === "signup-logs" && (
          <>
            ...
            <div className="hidden md:block bg-[#0a0a0a] border rounded-lg shadow-sm overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-100">
                  {logsLoading ? (
                    <tr>
                      <td colSpan={6} className="py-10 text-center text-[#555]">
                        Loading logs...
                      </td>
                    </tr>
                  ) : logs.length === 0 ? (
                    ...
                    logs.map((log) => {
                      ...
                      return (
                        <React.Fragment key={log.id}>
                          <tr
                            className={`hover:bg-[#050505] ${hasDetails ? "cursor-pointer" : ""} ${isExpanded ? "bg-[#050505]" : ""}`}
                          ...
                          >
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                ...
                                <div>
                                  ...
                                  {log.email && (
                                    <p className="text-xs text-[#555]">{_isDemo ? maskEmail(log.email) : log.email}</p>
                                  )}
                                </div>
                              </div>
                            </td>
                            <td className="px-4 py-3">
                              <span
                                className={`px-3 py-1 rounded-full text-xs font-semibold ${
                                  statusBadgeColors[log.status] || "bg-[#0a0a0a] text-[#e0e0e0]"
                                }`}
                              >
                            ...
                            <td className="px-4 py-3 text-sm">
                              {log.phoneNumber ? (_isDemo ? maskPhone(log.phoneNumber) : log.phoneNumber) : <span className="text-[#555]">—</span>}
                            </td>
                            ...
                            <tr>
                              <td colSpan={6} className="px-0 py-0">
                                <div className={`mx-4 my-2 p-4 rounded-lg text-sm space-y-3 ${log.status === "failed" ? "bg-red-50 border border-red-200" : log.status === "incomplete" ? "bg-yellow-50 border border-yellow-200" : "bg-green-50 border border-green-200"}`}>
                                  ...
                                  {log.errorMessage && (
                                    <div>
                                      <span className="font-semibold text-[#999]">Error Message:</span>
                                      <p className="text-red-700 mt-1">{log.errorMessage}</p>
                                    </div>
                                  )}
                                  {log.wabaId && (
                                    <div>
                                      <span className="font-semibold text-[#999]">WABA ID:</span>{" "}
                                      <span className="font-mono text-[#999]">{log.wabaId}</span>
                                    </div>
                                  )}
                                  {log.channelId && (
                                    <div>
                                      <span className="font-semibold text-[#999]">Channel ID:</span>{" "}
                                      <span className="font-mono text-[#999]">{log.channelId}</span>
                                    </div>
                                  )}
                                  {log.errorDetails && (
                                    <div>
                                      <span className="font-semibold text-[#999]">Raw Error Details (Meta API):</span>
                                      <pre className="mt-1 p-3 bg-[#0a0a0a] border rounded text-xs font-mono text-[#999] overflow-x-auto whitespace-pre-wrap">
            ...
            <div className="md:hidden space-y-4">
              {logsLoading ? (
                <p className="text-center text-[#555] py-10">Loading logs...</p>
              ) : logs.length === 0 ? (
                <p className="text-center text-[#555] py-10">No signup logs found</p>
              ) : (
                logs.map((log) => {
                  const hasDetails = log.errorMessage || log.errorDetails || log.wabaId;
                  const isExpanded = expandedLogId === log.id;
                  return (
                    <div
                      key={log.id}
                      className={`bg-[#0a0a0a] shadow-sm rounded-lg p-4 border space-y-3 ${hasDetails ? "cursor-pointer" : ""}`}
                      onClick={() => hasDetails && setExpandedLogId(isExpanded ? null : log.id)}
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                      ...
                      {isExpanded && (() => {
                        ...
                        <div className={`p-3 rounded-lg text-sm space-y-3 ${log.status === "failed" ? "bg-red-50 border border-red-200" : log.status === "incomplete" ? "bg-yellow-50 border border-yellow-200" : "bg-green-50 border border-green-200"}`}>
                          ...
                          {log.errorMessage && (
                            <div>
                              <span className="font-semibold text-[#999]">Full Error:</span>
                              <p className="text-red-700 mt-1">{log.errorMessage}</p>
                            </div>
                          )}
                          {log.wabaId && (
                          ...
                          {log.errorDetails && (
                            <div>
                              <span className="font-semibold text-[#999]">Raw Error Details (Meta API):</span>
                              <pre className="mt-1 p-2 bg-[#0a0a0a] border rounded text-xs font-mono text-[#999] overflow-x-auto whitespace-pre-wrap">
                                {JSON.stringify(log.errorDetails, null, 2)}
                              </pre>
                            </div>
                          )}
