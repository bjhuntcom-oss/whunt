      ...
      <div className="absolute top-4 right-4 flex items-center gap-2 z-10">
        {type === "image" && (
          <button
            ...
            className="p-2 bg-[#0a0a0a]/20 hover:bg-[#0a0a0a]/30 rounded-full transition-colors"
            title="Rotate 90°"
          >
...
const MessageItem = ({
        ...
        {hasMedia && mUrl && (
          <div
            className={cn(
              ...
              isOutbound ? "bg-[#c5e8b0]" : "bg-[#0a0a0a]"
            )}
          >
            <div
              className={cn(
            ...
            >
              ...
              >
                <FileText
                  className={cn(
                    "w-5 h-5",
                    isOutbound ? "text-[#999]" : "text-blue-600"
                  )}
                />
              </div>
              <div className="flex-1 min-w-0">
                <p
                  className={cn(
                    ...
                    isOutbound ? "text-[#e0e0e0]" : "text-[#e0e0e0]"
                  )}
                >
                  {fileName}
                </p>
              ...
              {dlUrl && (
                <a
                  ...
                  className={cn(
                    ...
                    isOutbound ? "hover:bg-[#a8d98a]" : "hover:bg-[#0a0a0a]"
                  )}
                  onClick={(e) => e.stopPropagation()}
                  title="Download file"
                >
                  ...
                  (button: { id?: string; text: string }, index: number) => (
                    <button
                      ...
                      className={cn(
                        ...
                        isOutbound
                          ? "border-[#a8d98a] text-[#999] hover:bg-[#c5e8b0]"
                          : "border-[#252525] text-[#999] hover:bg-[#050505]"
                      )}
                      onClick={() => {
            ...
            <div className="flex-1 min-w-0">
              <p
                className={cn(
                  ...
                  isOutbound ? "text-[#999]" : "text-blue-700"
                )}
              >
                Template Message
              </p>
        ...
        if (stickerUrl) {
          return (
            <div className="space-y-1">
              ...
              />
              <div className="hidden items-center gap-2 text-sm text-[#555]">
                <span>🏷️</span>
                <span>Sticker</span>
              </div>
            </div>
            ...
            {mapLink ? (
              <a
                ...
                className={cn(
                  ...
                  isOutbound ? "bg-[#c5e8b0] border-[#a8d98a]" : "bg-[#0a0a0a] border-[#1a1a1a]"
                )}
              >
                <div className="flex items-start gap-2">
                  <div className={cn(
                    ...
                    <MapPin className={cn("w-4 h-4", isOutbound ? "text-[#999]" : "text-red-500")} />
                  </div>
                  <div className="min-w-0 flex-1">
                    {locName && <p className="text-sm font-medium truncate">{locName}</p>}
                    {locAddress && <p className="text-xs text-[#555] truncate">{locAddress}</p>}
                    {lat && lng && (
                      <p className="text-xs text-[#555] mt-0.5">{Number(lat).toFixed(4)}, {Number(lng).toFixed(4)}</p>
                    )}
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  <div className={cn(
                    ...
                    <User className={cn("w-4 h-4", isOutbound ? "text-[#999]" : "text-blue-600")} />
                  </div>
                ...
                {contact.phones?.map((p, pidx) => (
                  <div key={pidx} className="flex items-center gap-1.5 text-xs text-[#999] mb-0.5">
                    ...
                    {p.type && <span className="text-[#555]">({p.type})</span>}
                  </div>
                ))}
                {contact.emails?.map((e, eidx) => (
                  <div key={eidx} className="flex items-center gap-1.5 text-xs text-[#999] mb-0.5">
      ...
      case "button": {
        ...
        return (
          <div className={cn(
            ...
            isOutbound ? "bg-[#c5e8b0]" : "bg-[#050505] border border-[#1a1a1a]"
          )}>
        ...
        return (
          <div className="space-y-1">
            <div className={cn(
              ...
              isOutbound ? "text-[#999]" : "text-green-700"
            )}>
              <span>📢</span>
      ...
      case "address": {
        return (
          <div className={cn(
            ...
            <MapPin className="w-4 h-4 text-[#555] flex-shrink-0" />
      ...
      case "location_request": {
        return (
          <div className={cn(
            ...
            <span className="text-[#999]">Location requested</span>
          </div>
        );
      }
      ...
      case "unsupported": {
        ...
        return (
          <div className={cn(
            ...
            isOutbound ? "bg-[#c5e8b0]" : "bg-[#050505]"
          )}>
            <div className="flex items-center gap-2">
              <HelpCircle className="w-4 h-4 text-[#555] flex-shrink-0" />
              <span className="text-[#555]">
                {errorTitle || "This message type is not supported in the platform"}
              </span>
            </div>
            {(originalType || errorCode || errorDetails) && (
              <div className="text-xs text-[#555] pl-6 space-y-0.5">
                {originalType && originalType !== "unsupported" && (
                  <p>Type: <span className="font-medium text-[#555]">{originalType}</span></p>
                )}
                {errorCode && (
                  <p>Error code: <span className="font-medium text-[#555]">{errorCode}</span></p>
                )}
                {errorDetails && (
                  <p>Details: <span className="font-medium text-[#555]">{errorDetails}</span></p>
                )}
              </div>
            )}
          </div>
  ...
  const getMessageStatusIcon = (status: string) => {
    switch (status) {
      case "sent":
        return <Check className="w-3 h-3 text-[#555]" />;
      case "delivered":
        return <CheckCheck className="w-3 h-3 text-[#555]" />;
      case "read":
        return <CheckCheck className="w-3 h-3 text-blue-500" />;
      case "failed":
        return <AlertCircle className="w-3 h-3 text-red-500" />;
      default:
        return <Clock className="w-3 h-3 text-[#555]" />;
    }
  };
    ...
    <>
      ...
      {showDate && (
        <div className="flex items-center justify-center my-4">
          <div className="bg-[#0a0a0a] px-3 py-1 rounded-full text-xs text-[#999]">
          ...
          className={cn(
            ...
            isOutbound
              ? message.status === "failed"
                ...
                : "bg-[#dcf8c6] text-[#e0e0e0] rounded-br-sm"
              : "bg-[#0a0a0a] text-[#e0e0e0] border border-[#1a1a1a] shadow-sm rounded-bl-sm"
          )}
        >
          {renderMediaContent()}
        ...
        {reactions && reactions.length > 0 && (
          <div className={cn(
            ...
            {Object.entries(
              ...
              <span
                key={emoji}
                className="inline-flex items-center bg-[#0a0a0a] border border-[#1a1a1a] rounded-full px-1.5 py-0.5 text-sm shadow-sm"
              >
                {emoji}
                {(count as number) > 1 && (
                  <span className="text-[10px] text-[#555] ml-0.5">{count as number}</span>
                )}
              </span>
            ))}
          </div>
