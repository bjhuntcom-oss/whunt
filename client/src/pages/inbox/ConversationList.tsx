...
const ConversationList = ({
  ...
  return (
    <div
      className={cn(
        "bg-[#0a0a0a] border-r border-[#1a1a1a] flex flex-col shadow-[2px_0_8px_-2px_rgba(0,0,0,0.06)]",
        selectedConversation
          ? "hidden md:flex md:w-[340px] lg:w-[400px]"
          : "w-full md:w-[340px] lg:w-[400px]"
      )}
    >
      <div className="p-2 sm:p-3 md:p-4 border-b border-[#1a1a1a] bg-[#0a0a0a]">
        <div className="relative mb-2 sm:mb-3">
          <Search className="absolute left-2 sm:left-3 top-1/2 transform -translate-y-1/2 h-3.5 w-3.5 sm:h-4 sm:w-4 text-[#555] pointer-events-none" />
          <Input
            placeholder="Search..."
            value={searchQuery}
            onChange={(e) => onSearchChange(e.target.value)}
            className="pl-7 sm:pl-9 pr-2 sm:pr-3 bg-[#050505] text-xs sm:text-sm w-full h-8 sm:h-10 rounded-lg"
          />
        </div>
        ...
        <Tabs value={filterTab} onValueChange={onFilterTabChange}>
          <div className="overflow-x-auto -mx-2 sm:-mx-3 md:-mx-4 px-2 sm:px-3 md:px-4 [&::-webkit-scrollbar]:h-[2px] [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-gray-300 [&::-webkit-scrollbar-thumb]:rounded-full">
            <TabsList className="inline-flex w-auto h-7 sm:h-9 md:h-10 gap-1 sm:gap-1.5 md:gap-2 bg-[#0a0a0a] p-0.5 sm:p-1 rounded-lg">
              <TabsTrigger
                value="all"
