              ...
              <CardContent>
                {isLoading ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-[#999]">Loading team members...</p>
                  </div>
                      ...
                      {teamMembers.map((member) => (
                        <Card key={member.id} className="overflow-hidden">
                          <CardContent className="p-4">
                            ...
                            {/* Last Active */}
                            <div className="flex items-center gap-2 text-sm text-[#999] pt-3 border-t">
                              <Clock className="h-4 w-4 text-[#555] flex-shrink-0" />
                              <span>
                                Last active:{" "}
                                {member.lastLogin
                                  ? new Date(
                    ...
                    {teamUserRes && (
                      <div className="mt-6">
                        ...
                        <div className="sm:hidden space-y-3">
                          <div className="text-center text-sm text-[#999] bg-[#050505] py-2 rounded-lg">
                            Page{" "}
                            <span className="font-medium">
                              {teamUserRes.page}
                            </span>{" "}
                ...
                <div className="md:hidden space-y-4 p-4">
                  {(activityLogs as any[])?.length === 0 ? (
                    ...
                    (activityLogs as any[]).map((log) => (
                      <div
                        key={log.id}
                        className="bg-[#0a0a0a] border rounded-lg p-4 shadow-sm space-y-3"
                      >
                        {/* Member */}
  ...
  const renderPermissionGroup = (group: PermissionGroup) => {
    ...
    return (
      <div key={group.title} className="border rounded-lg p-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <button
              ...
              className="p-1 hover:bg-[#0a0a0a] rounded"
            >
              {isExpanded ? (
        ...
        {isExpanded && (
          <div className="mt-3 ml-6 pl-4 border-l-2 border-[#1a1a1a]">
            <div className="grid grid-cols-3 gap-x-4 gap-y-2">
              {group.permissions.map((perm) => (
                <div key={perm.key} className="flex items-center space-x-2">
                  ...
                  <Label
                    htmlFor={perm.key}
                    className="text-xs text-[#999] cursor-pointer"
                  >
                    {perm.label}
                  </Label>
                </div>
