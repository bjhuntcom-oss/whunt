...
function TransactionsPage() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "cancelled":
        return <Ban className="w-4 h-4 text-[#555]" />;
      default:
        return null;
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    const colors: Record<string, string> = {
      ...
      cancelled: "bg-[#0a0a0a] text-[#e0e0e0]",
    };

    return (
      <span
        className={`px-2 py-1 rounded-full text-xs font-medium ${
          colors[status] || "bg-[#0a0a0a] text-[#e0e0e0]"
        }`}
      >
  ...
  return (
    <div className="min-h-screen bg-[#050505] dots-bg">
      {/* Header */}
      <Header
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 md:py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mb-6 sm:mb-8">
          <div className="bg-[#0a0a0a] rounded-lg shadow p-4 sm:p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs sm:text-sm text-[#999]">
                  {t("transactions.stats.totalRevenue")}
                </p>
                <p className="text-xl sm:text-2xl font-bold text-[#e0e0e0] mt-1">
          ...
          {["completed", "pending", "failed"].map((status) => {
            ...
            return (
              <div
                key={status}
                className="bg-[#0a0a0a] rounded-lg shadow p-4 sm:p-6 flex items-center justify-between"
              >
                <div>
                  <p className="text-xs sm:text-sm text-[#999] capitalize">
        ...
        {/* Filters */}
        <div className="bg-[#0a0a0a] rounded-lg shadow mb-4 sm:mb-6">
          <div className="p-3 sm:p-4 md:p-6">
            <div className="flex flex-col lg:flex-row gap-3 sm:gap-4">
              {/* Search */}
              <div className="flex-1 relative">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-[#555] w-4 h-4 sm:w-5 sm:h-5" />
                <input
                  type="text"
                  placeholder={t("transactions.search.placeholder")}
                  className="w-full pl-9 sm:pl-10 pr-4 py-2 sm:py-2.5 text-sm sm:text-base border border-[#252525] rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              ...
              <button
                ...
                className="px-3 sm:px-4 py-2 sm:py-2.5 text-sm sm:text-base border border-[#252525] rounded-lg hover:bg-[#050505] flex items-center justify-center gap-2"
              >
    ...
    {t("transactions.search.export")}
            ...
            {showFilters && (
              <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                {/* Status */}
                <div>
                  <label className="block text-xs sm:text-sm font-medium text-[#999] mb-2">
                ...
                <div>
                  ...
                  <input
                    type="date"
                    className="w-full px-3 py-2 text-sm border border-[#252525] rounded-lg focus:ring-2 focus:ring-blue-500"
                ...
                <div className="flex items-end">
                  <button
                    ...
                    className="w-full px-3 sm:px-4 py-2 text-sm border border-[#252525] rounded-lg hover:bg-[#050505] flex items-center justify-center gap-2"
                  >
                    <X className="w-4 h-4" />
        ...
        <div className="bg-[#0a0a0a] rounded-lg shadow overflow-hidden">
          {isLoading ? (
            ...
            <div className="text-center py-12">
              <CreditCard className="w-12 h-12 sm:w-16 sm:h-16 text-[#555] mx-auto mb-4" />
            ...
            <>
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-[#050505] border-b">
                    <tr>
                      <th className="px-3 lg:px-6 py-3 text-left text-xs font-medium text-[#555] uppercase tracking-wider">
                      ...
                      <th className="px-3 lg:px-6 py-3 text-left text-xs font-medium text-[#555] uppercase tracking-wider hidden lg:table-cell">
                  ...
                  <tbody className="bg-[#0a0a0a] divide-y divide-gray-200">
                    {transactions.map((item: any) => (
                      <tr
                        ...
                        className="hover:bg-[#050505]"
                      >
                        <td className="px-3 lg:px-6 py-3 lg:py-4 whitespace-nowrap">
                          <div className="text-xs sm:text-sm font-medium text-[#e0e0e0]">
                        ...
                        <td className="px-3 lg:px-6 py-3 lg:py-4 whitespace-nowrap">
                          <div className="text-xs sm:text-sm text-[#e0e0e0] max-w-[150px] truncate">
                            {isDemoUser(user?.username) ? maskValue(item.user?.email || "") : item.user?.email}

                          </div>
                        </td>
                        <td className="px-3 lg:px-6 py-3 lg:py-4 whitespace-nowrap">
                          <div className="text-xs sm:text-sm text-[#e0e0e0]">
                            {item.plan?.name}
                          </div>
                          <div className="text-xs text-[#555] capitalize">
                        ...
                        <td className="px-3 lg:px-6 py-3 lg:py-4 whitespace-nowrap hidden lg:table-cell">
                          <div className="text-sm text-[#e0e0e0] capitalize">
                            {item.provider.name}
                          </div>
                        </td>
                        <td className="px-3 lg:px-6 py-3 lg:py-4 whitespace-nowrap text-xs sm:text-sm text-[#555]">
              ...
              <div className="md:hidden space-y-3 p-3">
                {transactions.map((item: any) => (
                  <div
                    key={item.transaction.id}
                    className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-lg p-4 shadow-sm"
                  >
                    <div className="flex items-start justify-between mb-3 gap-2">
                      <div className="flex-1 min-w-0">
                        <div className="text-xs font-mono text-[#e0e0e0] font-medium truncate">
                          {item.transaction.providerTransactionId}
                        </div>
                    ...
                    <div className="space-y-2 text-sm border-t border-[#1a1a1a] pt-3">
                      <div className="flex justify-between items-start">
                        ...
                        <span className="text-[#e0e0e0] text-xs font-medium text-right truncate ml-2 max-w-[60%]">
                          {item.user?.email}
                        </span>
                      </div>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-[#e0e0e0] text-xs capitalize">
                          {item.transaction.billingCycle}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        ...
                        <span className="text-[#e0e0e0] text-xs font-bold">
                          {item.transaction.currency}{" "}
                          {Number(item.transaction.amount).toFixed(2)}
                        </span>
                      </div>
              ...
              {/* Pagination */}
              <div className="bg-[#050505] px-3 sm:px-4 md:px-6 py-3 sm:py-4 border-t">
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
                  <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-4 w-full sm:w-auto">
                    ...
                    <select
                      ...
                      className="border border-[#252525] rounded-md px-2 py-1 text-xs sm:text-sm"
                    >
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto justify-center">
                    <button
                      ...
                      className="px-3 py-2 text-xs sm:text-sm border border-[#252525] rounded-lg hover:bg-[#0a0a0a] disabled:opacity-50 flex items-center gap-1"
                    >
