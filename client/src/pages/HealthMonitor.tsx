...
const HealthMonitor = () => {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      default:
        return <Clock className="w-4 h-4 sm:w-5 sm:h-5 text-[#555]" />;
    }
  };
  ...
  const getTrendIcon = (trend: string) => {
    switch (trend) {
      ...
      default:
        return <Activity className="w-3 h-3 sm:w-4 sm:h-4 text-[#555]" />;
    }
  };
  ...
  const getComponentIcon = (name: string) => {
    if (name.includes("WhatsApp"))
      return <Smartphone className="w-4 h-4 sm:w-5 sm:h-5 text-[#555]" />;
    if (name.includes("Message"))
      return <MessageSquare className="w-4 h-4 sm:w-5 sm:h-5 text-[#555]" />;
    if (name.includes("Template"))
      return <FileText className="w-4 h-4 sm:w-5 sm:h-5 text-[#555]" />;
    if (name.includes("Contact"))
      return <Users className="w-4 h-4 sm:w-5 sm:h-5 text-[#555]" />;
    if (name.includes("Analytics"))
      return <Activity className="w-4 h-4 sm:w-5 sm:h-5 text-[#555]" />;
    if (name.includes("Webhook"))
      return <Globe className="w-4 h-4 sm:w-5 sm:h-5 text-[#555]" />;
    return <Server className="w-4 h-4 sm:w-5 sm:h-5 text-[#555]" />;
  };
  ...
  return (
    <div className="min-h-screen bg-[#050505]">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center py-3 sm:py-4 gap-3">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <select
                ...
                className="flex-1 sm:flex-none px-3 sm:px-4 py-2 text-sm sm:text-base border border-[#252525] rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
              >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div className="bg-[#0a0a0a] p-4 sm:p-6 rounded-xl shadow-sm border border-[#1a1a1a] mb-6 sm:mb-8">
          <div className="flex flex-col gap-4 mb-4 sm:mb-6">
            <div className="flex items-start space-x-3 sm:space-x-4">
              ...
              <div className="flex-1 min-w-0">
                <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-[#e0e0e0]">
                  {systemStatus.overall === "healthy"
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:space-x-3">
              <button className="w-full sm:w-auto px-3 sm:px-4 py-2 text-sm sm:text-base border border-[#252525] rounded-lg hover:bg-[#050505] transition-colors flex items-center justify-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {systemStatus.components.map((component, index) => (
              <div
                key={index}
                className="border border-[#1a1a1a] rounded-lg p-3 sm:p-4 hover:shadow-sm transition-all"
              >
                <div className="flex items-center justify-between mb-2 sm:mb-3">
                  <div className="flex items-center space-x-2 sm:space-x-3 flex-1 min-w-0">
                    {getComponentIcon(component.name)}
                    <h3 className="font-medium text-sm sm:text-base text-[#e0e0e0] truncate">
                      {component.name}
                    </h3>
                  </div>
                  <span
                ...
                <div className="grid grid-cols-2 gap-2 text-xs sm:text-sm">
                  <div>
                    ...
                    <span className="ml-1 sm:ml-2 font-medium text-[#e0e0e0]">
                      {component.uptime}
                    </span>
                  </div>
                  <div>
                  ...
                  <div className="col-span-2 sm:col-span-1">
                    <span className="text-[#555]">Last Incident:</span>
                    <span className="ml-1 sm:ml-2 font-medium text-[#e0e0e0]">
                      {component.lastIncident}
                    </span>
                  </div>
                  <div className="flex items-center col-span-2 sm:col-span-1">
                    <span className="text-[#555]">Trend:</span>
                    <span className="ml-1 sm:ml-2 flex items-center">
                      {getTrendIcon(component.trend)}
                      <span className="ml-1 font-medium text-[#e0e0e0]">
                        {component.trend}
                      </span>
                    </span>
                  </div>
          ...
          {metrics.map((metric, index) => (
            ...
            >
              ...
              <div className="flex items-end justify-between">
                <div className="text-2xl sm:text-3xl font-bold text-[#e0e0e0]">
                  {metric.value}
                </div>
                <div
        ...
        {incidents.length > 0 && (
          <div className="bg-[#0a0a0a] rounded-xl shadow-sm border border-[#1a1a1a] overflow-hidden mb-6 sm:mb-8">
            <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-[#1a1a1a] bg-[#050505]">
            ...
            <div className="divide-y divide-gray-200">
              {incidents.map((incident) => (
                <div key={incident.id} className="p-4 sm:p-6">
                  ...
                  <p className="text-sm sm:text-base text-[#999] mb-3 sm:mb-4">
                    {incident.description}
                  </p>

                  <div className="bg-[#050505] p-3 sm:p-4 rounded-lg">
                    <h5 className="font-medium text-sm sm:text-base text-[#e0e0e0] mb-2">
                      Updates
                    </h5>
          ...
          <div className="bg-[#0a0a0a] p-4 sm:p-6 rounded-xl shadow-sm border border-[#1a1a1a]">
            ...
            <div className="space-y-3 sm:space-y-4">
              {[
                ...
                <div key={index} className="bg-[#050505] p-3 sm:p-4 rounded-lg">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2">
                    <h4 className="font-medium text-sm sm:text-base text-[#e0e0e0]">
                      {api.name}
                    </h4>
                ...
                <div key={index} className="bg-[#050505] p-3 sm:p-4 rounded-lg">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-xs sm:text-sm font-medium text-[#e0e0e0] whitespace-nowrap">
                        {resource.value}
