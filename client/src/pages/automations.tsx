...
export default function Automations() {
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="h-4 w-72 bg-[#0a0a0a] rounded animate-pulse" />
          </div>
          <div className="h-10 w-36 bg-gray-200 rounded-lg animate-pulse" />
        </div>
        <div className="grid grid-cols-3 gap-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-lg p-4 animate-pulse">
              <div className="h-4 w-20 bg-[#0a0a0a] rounded mb-2" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-lg p-5 animate-pulse">
              <div className="h-5 w-40 bg-gray-200 rounded mb-3" />
              <div className="h-4 w-full bg-[#0a0a0a] rounded mb-2" />
              <div className="h-4 w-2/3 bg-[#0a0a0a] rounded mb-4" />
              <div className="flex gap-2 pt-3 border-t border-[#1a1a1a]">
                <div className="h-8 flex-1 bg-[#0a0a0a] rounded" />
                <div className="h-8 flex-1 bg-[#0a0a0a] rounded" />
                <div className="h-8 flex-1 bg-[#0a0a0a] rounded" />
              </div>
            </div>
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-[#e0e0e0]">Automation Flows</h1>
          <p className="text-sm text-[#555] mt-0.5">Create and manage conversation flows with drag-and-drop visual builder</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-lg p-4 flex items-center gap-3">
          <div className="h-9 w-9 rounded-lg bg-blue-50 flex items-center justify-center">
            <Layers className="h-4 w-4 text-blue-600" />
          </div>
          <div>
            <p className="text-xs text-[#555]">Total Flows</p>
            <p className="text-lg font-semibold text-[#e0e0e0]">{automations.length}</p>
          </div>
        </div>
        <div className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-lg p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-[#555]">Active Flows</p>
            <p className="text-lg font-semibold text-[#e0e0e0]">{activeCount}</p>
          </div>
        </div>
        <div className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-lg p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-[#555]">Total Executions</p>
            <p className="text-lg font-semibold text-[#e0e0e0]">{totalExecutions}</p>
          </div>
        </div>
      </div>
      ...
      {drafts.length > 0 && (
        <div className="space-y-3">
          <h2 className="text-sm font-semibold text-[#999] flex items-center gap-2">
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    size="sm"
                    variant="ghost"
                    className="h-8 w-8 p-0 text-[#555] hover:text-red-500 hover:bg-red-50"
      ...
      {automations.length === 0 && drafts.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 bg-[#0a0a0a] border border-[#1a1a1a] rounded-lg">
          <div className="h-16 w-16 rounded-full bg-[#0a0a0a] flex items-center justify-center mb-4">
            <Workflow className="h-7 w-7 text-[#555]" />
          </div>
          <h3 className="text-base font-semibold text-[#e0e0e0] mb-1">
            {t("automations.empityAuto.title")}
          </h3>
          <p className="text-sm text-[#555] max-w-sm text-center mb-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {automations.map((automation: any) => {
            ...
            return (
              <div
                key={automation.id}
                className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-lg hover:border-[#252525] hover:shadow-sm transition-all duration-200"
              ...
              >
                <div className="p-4 pb-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-2 min-w-0 flex-1">
                      ...
                      <h3
                        className="font-medium text-sm text-[#e0e0e0] truncate"
                ...
                <div className="px-4 pb-3">
                  {automation.description ? (
                    <p
                      className="text-xs text-[#555] line-clamp-2 mb-3"
                      data-testid={`text-description-${automation.id}`}
                    >
                      {automation.description}
                    </p>
                  ) : (
                    <p className="text-xs text-[#555] italic mb-3">
                      No description
                    </p>
                  )}

                  <div className="flex items-center gap-3 mb-3">
                    <span className="inline-flex items-center gap-1 text-[11px] font-medium bg-[#0a0a0a] text-[#999] px-2 py-0.5 rounded">
                      <Zap className="h-3 w-3" />
                      {automation.trigger || "New Chat"}
                    </span>
                    <Badge
                      variant={isActive ? "default" : "secondary"}
                      className={`text-[10px] px-1.5 py-0 rounded font-medium ${isActive ? "bg-green-100 text-green-700 hover:bg-green-100 border-0" : "bg-[#0a0a0a] text-[#555] hover:bg-[#0a0a0a] border-0"}`}
                    >
                ...
                <div className="flex items-center border-t border-[#1a1a1a]">
                  <Button
                    variant="ghost"
                    size="sm"
                    className="flex-1 text-xs h-9 text-[#999] hover:text-blue-600 hover:bg-blue-50/50 rounded-none font-medium"
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    className="flex-1 text-xs h-9 text-[#999] hover:text-amber-600 hover:bg-amber-50/50 rounded-none font-medium"
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    className="flex-1 text-xs h-9 text-[#999] hover:text-red-600 hover:bg-red-50/50 rounded-none font-medium"
                    onClick={() => {
                      if (
                        confirm(
