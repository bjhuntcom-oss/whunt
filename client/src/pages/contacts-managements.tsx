              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  <DropdownMenuItem
                    ...
                    className={selectedStatus === "active" ? "bg-[#0a0a0a]" : ""}
                  >
                    {t("contacts.active")}
                  </DropdownMenuItem>
                  <DropdownMenuItem
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card>
              <CardContent className="p-4 text-center">
                <p className="text-2xl font-bold text-[#e0e0e0]">{contactStats.total.toLocaleString()}</p>
                <p className="text-sm text-[#555] mt-1">Total Contacts</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 text-center">
                <p className="text-2xl font-bold text-blue-600">{contactStats.uniquePhones.toLocaleString()}</p>
                <p className="text-sm text-[#555] mt-1">Unique Phones</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 text-center">
                <p className="text-2xl font-bold text-green-600">{contactStats.activeCount.toLocaleString()}</p>
                <p className="text-sm text-[#555] mt-1">Active</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 text-center">
                <p className="text-2xl font-bold text-red-600">{contactStats.blockedCount.toLocaleString()}</p>
                <p className="text-sm text-[#555] mt-1">Blocked</p>
              </CardContent>
            </Card>
          </div>
        )}
            ...
            {!contacts.length ? (
              ...
              <>
                ...
                <div className="hidden lg:block overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-[#050505]">
                      <tr>
                        <th className="text-left px-4 xl:px-6 py-4 text-xs font-medium text-[#555] uppercase tracking-wider">
                          <input
                            type="checkbox"
                ...
                <div className="lg:hidden divide-y divide-gray-200">
                  {/* Select All Bar */}
                  <div className="bg-[#050505] px-4 py-3 flex items-center justify-between">
                    <label className="flex items-center gap-2 text-sm font-medium text-[#999]">
                      <input
                        type="checkbox"
                        className="rounded border-[#252525]"
                        checked={allSelected}
                  ...
                  {/* Contact Cards */}
                  {contacts.map((contact: Contact) => (
                    <div key={contact.id} className="bg-[#0a0a0a] p-4">
                      <div className="flex items-start gap-3">
                        {/* Checkbox */}
                        <input
                          type="checkbox"
                          className="rounded border-[#252525] mt-1"
                          checked={selectedContactIds.includes(contact.id)}
                          onChange={() => toggleSelectOne(contact.id)}
                        />
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between mb-2">
                            <div className="flex-1 min-w-0">
                              <h3 className="text-base font-semibold text-[#e0e0e0] truncate">
                                {isDemoUser(user?.username) ? maskName(contact.name) : contact.name}
                              </h3>
                              {contact.email && (
